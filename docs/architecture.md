...
normalization.
HDMA destinations wrap within the selected 8 KiB VRAM bank at the 9FFF/8000
boundary.
...
and FF55 retains the inactive bit plus the remaining-block count;
...
can restart the window only at a future WX boundary. On DMG/MGB, a window tile
fetch already in progress retains its full restart timing and emits the fetched
eight-pixel tile before background output resumes. Native CGB-mode stall nuances
remain separate.
...
progression pauses while HALT continues to clock the timer, matching SameBoy's
separate stopped and halted CPU states.
...
set, while DMG and normal CGB transfers retain the 4096-T-cycle cadence.
...
32-byte nibble-addressed EEPROM command path plus TAMA6 timer, alarm, and free
RTC pages with injected host-time advancement and battery persistence. Pinned
...
Later color revisions share the same initial double-speed read window before
...
Later CGB revisions also keep OAM reads blocked for the first normal-speed
HBlank T-cycle, while OAM writes remain available.
...
SGB external-buffer descriptors are checked against the complete file before
