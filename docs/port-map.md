...
| `Core/timing.c` | `Scheduler`, `EmulatorState`, and cycle execution authority | partial | per-instruction native T-cycle comparisons; participant scheduler tests; prioritized interrupt dispatch and 20-T-cycle service tests; IF/IE interrupt-register bus behavior and high-bit wake/dispatch edge coverage |
...
| `Core/display.c` | `PpuDevice` | partial | LCD mode timing/reset, SCX fine-scroll and WX=0 window-fetch mode-3 penalties, edge-triggered LY/LYC/STAT interrupts, CPU VRAM/OAM access blocking, CGB VBK bank selection and indexed BG/OBJ palette RAM registers, DMG background/window/sprite tile and palette rendering including 8×16 sprites and overlap priority, and raw frame-buffer tests |
