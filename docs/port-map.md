...
| `Core/sm83_cpu.c` | `Emulator.Execute` | partial | register-transfer block, register ADC/SBC/XOR, indirect HL jump and STOP, immediate accumulator ALU, signed SP-relative operations, 16-bit pair arithmetic, accumulator rotates/status, delayed EI/DI control and interrupt-boundary state hashing, conditional calls/returns, RETI and RST, `(HL)` INC/DEC, explicit immediate load/store, ALU, INC/DEC, CB-prefixed rotate/shift, BIT/RES/SET, relative/absolute branch, stack, CALL/RET instructions with per-instruction native comparisons |
