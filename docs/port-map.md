...
| `Core/sm83_cpu.c` | `Emulator.Execute` | partial | register-transfer block, immediate accumulator ALU, `(HL)` INC/DEC, explicit immediate load/store, ALU, INC/DEC, CB-prefixed rotate/shift, BIT/RES/SET, relative/absolute branch, stack, CALL/RET instructions with per-instruction native comparisons |
