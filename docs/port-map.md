...
| `Core/sm83_cpu.c` | `Emulator.Execute` | partial | register-transfer block, register ADC/SBC/XOR, indirect HL jump and STOP, HALT state and HALT-bug hashing, immediate accumulator ALU, signed SP-relative operations, 16-bit pair arithmetic, accumulator rotates/status, delayed EI/DI control and interrupt-boundary state hashing, conditional calls/returns, RETI and RST, `(HL)` INC/DEC, explicit immediate load/store, ALU, INC/DEC, CB-prefixed rotate/shift, BIT/RES/SET, relative/absolute branch, stack, CALL/RET instructions, and SameBoy-compatible illegal-opcode halt behavior with per-instruction native comparisons |
