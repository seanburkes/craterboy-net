...
| `Core/save_state.c` | `InputRecording` / BESS | partial | BESS structural reader validates the little-endian footer, block bounds, required CORE/END structure, duplicate CORE, known-block ordering, and empty END while skipping unknown blocks; versioned streaming input-recording reads from seekable/forward-only sources and writes to seekable/forward-only destinations, with read-only event exposure, null-argument validation, caller-owned stream lifetime and primary-player scope, exact-cycle replay, DMG/MGB/CGB checkpoint-hash replay coverage, header and event-count bounds, ordering and invalid-field/truncation/trailing-data validation with a uniform `InvalidDataException` parse contract, and deterministic hashing including cartridge state; field-level BESS state remains deferred |
