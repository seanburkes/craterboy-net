...
| `Core/camera.c`, camera portions of `Core/memory.c` | Pocket Camera cartridge | partial | MBC5-style ROM/RAM banking, register aperture/masks/readback, busy-state RAM gating, battery storage, and pinned SameBoy differential coverage; image acquisition, image processing, and capture-completion timing remain deferred |
| printer, rumble, WorkBoy | accessories | deferred | — |
